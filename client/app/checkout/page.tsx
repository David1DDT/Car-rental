...
const CheckoutPage = ({ searchParams }: { searchParams: Promise<{ carId: string, startDate: string, endDate: string, loc: string }> }) => {
    ...
    const handleSumbit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        if (!formData.email || !formData.phone || !formData.name || !formData.address || (customerType === "juridical" && (!formData.companyName || !formData.registrationNumber))) {
    ...
    return (
        <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex flex-col">
            <div className="max-w-2xl mx-auto w-full flex-1 py-12 px-4">
